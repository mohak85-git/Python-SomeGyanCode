    ...
    if bmp_id == b'BM':  # we have a Windows bitmap file
        ...
        if file_size != os_size:
            ...
            if dib_header_size == 40:
                image_width = int.from_bytes(dib_header_etc[4:8], 'little',
                                             signed=True)
                image_height = int.from_bytes(dib_header_etc[8:12], 'little',
                                              signed=True)
                ...
                if current_position != offset:
                    print(f"Something's gone wrong. We're at {current_position}, "
                          f"should be at {offset}")
